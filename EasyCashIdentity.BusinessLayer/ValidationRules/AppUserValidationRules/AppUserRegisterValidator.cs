...
namespace EasyCashIdentity.BusinessLayer.ValidationRules.AppUserValidationRules
{
	public class AppUserRegisterValidator: AbstractValidator<AppUserRegisterDto>
	{
        public AppUserRegisterValidator()
        {
            ...
            RuleFor(x => x.Email).NotEmpty().WithMessage("Kullanıcı adı alanı boş geçilemez!");
            RuleFor(x => x.Password).NotEmpty().WithMessage("Şifre alanı boş geçilemez!");
            RuleFor(x => x.ConfirmPassword).NotEmpty().WithMessage("Şifre tekrar alanı boş geçilemez!");
            RuleFor(x => x.Name).MaximumLength(30).WithMessage("Lütfen en fazla 30 karakter girişi yapın!");
            RuleFor(x => x.Name).MinimumLength(2).WithMessage("Lütfen en az 2 karakter girşi yapın!");
